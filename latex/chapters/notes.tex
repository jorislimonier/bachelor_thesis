...
\begin{definition}[Compatibility condition]
    ...
    \begin{equation}
        \left\| \beta_{S_{0}} \right\|_{1}^{2} \leq \left(\beta^{T} \hat{\Sigma} \beta\right) \frac{s_{0}}{\phi_{0}^{2}}
    \end{equation}
\end{definition}
...
\begin{definition}[Compatibility condition for general sets]
    ...
    $$
        \left\|\beta_{S}\right\|_{1}^{2} \leq\left(\beta^{T} \hat{\Sigma} \beta\right)\frac{|S|}{\phi^{2}(S)}
    $$
\end{definition}
...
\begin{definition}[The oracle]
    We define the oracle $\beta^{*}$ as
    $$
        \beta^{*} = \arg \min_{\beta: S_{\beta} \in \mathscr{S}} \left\{ \frac{ \left\| \X \beta - \f^{0} \right\|_{2}^{2}}{n} + \frac{4 \lambda^{2} s_{\beta}}{\phi^{2}\left(S_{\beta}\right)}\right\}
    $$
    where $S_{\beta}:=\left\{j: \beta_{j} \neq 0\right\}$, $s_{\beta}:=\left|S_{\beta}\right|$ denotes the cardinality of $S_{\beta}$ and the factor 4 in the right hand side comes from choosing $\lambda \geq \lambda_0$.
\end{definition}
